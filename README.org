# -*- coding: utf-8 -*-
#+TITLE: EZPWD Reed-Solomon Version 1.5.3
...
*** Javascript Hosted by MaxCDN
   ...
   [[ https://cdn.rawgit.com/pjkundert/ezpwd-reed-solomon/v1.5.2/js/ezpwd/ezcod.js]].
    ...
    #+BEGIN_SRC HTML
    <script
      src="//cdn.rawgit.com/pjkundert/ezpwd-reed-solomon/v1.5.2/js/ezpwd/ezcod.js">
    </script>
    #+END_SRC
...
** ezpwd::RS<...>: C++ Reed-Solomon API
   ...
   data.resize( data.size() - rs.nroots() );	// Ddiscard the 4 R-S parity symbols
   #+END_SRC
...
*** EZCOD REST API Demo
    ...
    the API is hosted at: http://localhost:8000/api/v1.5.2.  To get the details
    for an EZCOD, encode a request with the EZCOD as a query option.  For
    example, visit this with a web browser:
    http://localhost:8000/api/v1.5.2?ezcod=r3u08mpvt.d.  This will return the
    decoded data as HTML.  To get it in JSON form, append =.json= to the API
    requests path: http://localhost:8000/api/v1.5.2.json?ezcod=r3u08mpvt.d.
