...
// Use 32 roots and the CCSDS initialization terms, for compatibility
// with Phil's fastest en/decode_rs_8 fixed-size decoder.  CCSDS only
// defined for 32 and 16 parity; only implemented by Phil Karn w/ 32
// parity.

//        eg.     255,            32,            223
template <size_t TOTAL, size_t ROOTS, size_t PAYLOAD=TOTAL-ROOTS, int POLY=0x187, int FCR=112, int PRIM=11>
double 				compare_ccsds(
				    ezpwd::asserter    &assert )
{
	...
	std::cout
	    << assert << " EZPWD and Karn R-S encoders produced different parity"
	    << std::endl;
...
// Standard R-S codec
//        eg.     255,            32,            223
template <size_t TOTAL, size_t ROOTS, size_t PAYLOAD=TOTAL-ROOTS, int POLY=0x11d, int FCR=1, int PRIM=1>
double 				compare(
				    ezpwd::asserter    &assert )
{
    std::array<uint8_t,TOTAL>	orig;
    ...
    // Ensure that each RS encoder produces the same parity symbols; both Phil Karn's generic codec,
    // and the new ezpwd/reed_solomon codec.
    void		       *grs	= init_rs_char( 8, POLY, FCR, PRIM, ROOTS, 0 );
    encode_rs_char( grs, orig.data(), orig.data() + PAYLOAD );
    std::array<uint8_t,TOTAL>	gdata( orig );
#if defined( DEBUG )
    ...
    /* Finite Field Parameters */
    const std::size_t field_descriptor                 =   8;
    const std::size_t generator_polynommial_index      =   1;
    const std::size_t generator_polynommial_root_count =  ROOTS;
...
int main()
{
    ...
    avg				       += compare_ccsds<255, 32>( assert );++cnt;
    avg				       += compare<255, 32>( assert );	++cnt;
